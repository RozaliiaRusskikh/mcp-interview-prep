...
**Monolith structure**: MCP's client/server split is a real process boundary (FastAPI launches `backend/mcp/server.py` as a stdio subprocess), so it can't fully merge away — but it stays an implementation detail *inside* the backend, not a third service. `mcp/` lives *inside* `backend/` (not a sibling folder) specifically because Render deploys the backend with root directory `backend/`, and Render has no access to files outside a service's configured root — a sibling `mcp/` would be invisible to the deployed service. One repo, subfolders (`backend/` — containing `backend/mcp/` — and `frontend/`), and exactly **two deployable units**: the backend (FastAPI process that spawns the MCP server itself) and the frontend (static React build). Deployed and operated like a normal two-tier app.
...
- **Phase 2 (later)**: build the FastAPI backend as a custom MCP client connecting to this same `backend/mcp/server.py`. It calls the MCP tools/resources, then passes that context to Gemini. The React frontend only talks to FastAPI over HTTP — it never talks to MCP directly.
...
**Data files** (JSON, in `backend/mcp/data/`):
...
**`backend/mcp/server.py`** (`mcp.server.fastmcp.FastMCP`), one shared `_load_json(path)` helper reused everywhere:
...
**Phase 1 status**: `backend/mcp/server.py`, `.mcp.json`, and `backend/mcp/data/*.json` are built; unit tests in `backend/mcp/tests/test_server.py` (pytest) cover the happy/not-found path for every tool plus the resources and prompts — all passing. Verified standalone via `uv run mcp dev backend/mcp/server.py` (MCP Inspector). Remaining: live verification through Claude Code itself.
...
2. **MCP client connection** — use the `mcp` SDK's stdio client to launch `backend/mcp/server.py` as a subprocess and open a session (same pattern as Claude Code does, just in your own code instead of Claude Code's host).
...
5. **Run the eval suite offline, gated in CI** — triggered manually or as a CI step before deploy (`pytest`-style, alongside the existing `backend/mcp/tests/` suite), against the fixed versioned dataset, not on live production traffic. `langfuse` dataset run, scoring every test case via the tiered logic above, producing one aggregate score checked against the 0.8 threshold — fail the pipeline if it's not met. Live/online eval on real user questions is a possible future addition, not part of this pass.
