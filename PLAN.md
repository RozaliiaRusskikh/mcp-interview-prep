...
5. **Run the eval suite offline, gated in CI** — triggered manually or as a CI step before deploy (`pytest`-style, alongside the existing `backend/mcp_server/tests/` suite), against the fixed versioned dataset, not on live production traffic. `langfuse` dataset run, scoring every test case via the tiered logic above, producing one aggregate score checked against the 0.8 threshold — fail the pipeline if it's not met. Live/online eval on real user questions is a possible future addition, not part of this pass.
