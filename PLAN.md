...
- **LLM**: Gemini via `google-genai` SDK (Google's current SDK — `google-generativeai` is deprecated/unmaintained) — Roza's own free-tier key, server-side only. No bring-your-own-key flow: recruiters are the audience, and requiring them to get an API key before they can chat is friction that defeats the point of the demo.
...
**Monolith structure**: MCP's client/server split is a real process boundary (FastAPI launches `backend/mcp_server/server.py` as a stdio subprocess), so it can't fully merge away — but it stays an implementation detail *inside* the backend, not a third service. `mcp_server/` lives *inside* `backend/` (not a sibling folder) specifically because Render deploys the backend with root directory `backend/`, and Render has no access to files outside a service's configured root — a sibling `mcp_server/` would be invisible to the deployed service. One repo, subfolders (`backend/` — containing `backend/mcp_server/` — and `frontend/`), and exactly **two deployable units**: the backend (FastAPI process that spawns the MCP server itself) and the frontend (static React build). Deployed and operated like a normal two-tier app.
...
- **Phase 2 (later)**: build the FastAPI backend as a custom MCP client connecting to this same `backend/mcp_server/server.py`. It calls the MCP tools/resources, then passes that context to Gemini. The React frontend only talks to FastAPI over HTTP — it never talks to MCP directly.
...
**Data files** (JSON, in `backend/mcp_server/data/`):
...
**`backend/mcp_server/server.py`** (`mcp.server.fastmcp.FastMCP`), one shared `_load_json(path)` helper reused everywhere:
...
- **Prompts** (reusable workflows):
  ...
  - `answer_as_roza()` → persona/tone/boundary instructions only (no question argument — loaded from `personal.json` in Python, not a separate fetch), meant to be used as the model's system instruction with the visitor's question passed separately as user content, instructing the model to answer in first person, grounded in the get_situation/get_experience/get_skill/get_contact tools or resources
...
**Phase 1 status**: `backend/mcp_server/server.py`, `.mcp.json`, and `backend/mcp_server/data/*.json` are built; unit tests in `backend/mcp_server/tests/test_server.py` (pytest) cover the happy/not-found path for every tool plus the resources and prompts — all passing. Verified standalone via `uv run mcp dev backend/mcp_server/server.py` (MCP Inspector). Remaining: live verification through Claude Code itself.

## Phase 2 — detailed steps

### Backend (`backend/` folder, FastAPI)
1. `uv add fastapi uvicorn google-genai mcp pydantic` in a separate `pyproject.toml` under `backend/` (its own package, own venv/lock — matches the two-deployable-units structure); `pydantic` ships with FastAPI but pin it explicitly since schemas are a deliberate practice goal here.
2. **MCP client connection** — use the `mcp` SDK's stdio client to launch `backend/mcp_server/server.py` as a subprocess and open a session (same pattern as Claude Code does, just in your own code instead of Claude Code's host).
...
6. **LLM fallback** — only reached when no keyword matches: render the MCP `answer_as_roza` prompt (persona/tone/boundaries, built from `personal.json`) as Gemini's `system_instruction`, send the visitor's question as separate user content — a structural trusted/untrusted split, not a text label inside one combined message — then call Gemini via `google-genai`, using Roza's own `GOOGLE_API_KEY` (server-side env var/secret, never sent to the frontend).
...
1. **Instrument `/chat`** — wrap each request in a Langfuse trace (question in, router decision, tool call + arguments, tool result, final answer out). `langfuse` Python SDK, just decorators/context managers around the existing Phase 2 code — no architecture change. For adversarial-style questions, also attach a named score to the trace itself (e.g. `"prompt-injection-detected": true/false`, from the same decline-signal check the eval run uses) — this makes a jailbreak attempt visible on its individual trace in the Langfuse dashboard directly, not just folded into a batch aggregate score. (Borrowed from Langfuse's own security-monitoring cookbook pattern — just the per-trace scoring idea, not its guardrail-library dependencies, which aren't needed here since defense is already handled by the system prompt.)
2. **Build a versioned eval dataset** — checked into the repo (e.g. `eval/dataset.json`) and synced to Langfuse, so changes to test cases are tracked in git like any other code change:
   ...
   - LLM fallback cases: open-ended question → `expected_tool_calls` (which tool(s) the model must actually call — a trajectory check here too, not just router cases) plus a short rubric of facts that must appear (e.g. "mentions El Paso Labs", "mentions FastAPI") — tests the Gemini path for hallucination/drift.
3. **Deterministic scoring only — no LLM-as-judge** (every case in the dataset is expressible as a plain check, so a judge call would be pure added cost/latency/non-determinism for no benefit):
   ...
   - LLM fallback cases, trajectory: `expected_tool_calls` checked against the Langfuse trace — did the model actually call a tool, or answer from pattern-completion alone? This is the check that would have caught the real "Austin, TX" relocation hallucination and the AI/LLM-systems fabrication directly, rather than relying on catching the wrong output text after the fact.
   - LLM fallback cases, output: keyword/fact-presence check (`expected_facts`, does the answer contain the required facts as substrings) and fact-absence check (`must_not_contain`) — cheap, deterministic, catches most regressions (missing facts, wrong company name, fabricated specifics, etc.).
   - Ordered-fact cases (e.g. ranked skill lists): substring-index comparison (`answer.index(a) < answer.index(b)`) instead of an LLM judge — still deterministic.
4. **Pass threshold = 0.9** — a dataset run passes if ≥90% of cases score correct (trajectory accuracy + fallback pass-rate combined). Below 0.9 = regression, flagged before deploying.
5. **Run the eval suite offline, gated in CI** — triggered manually or as a CI step before deploy (`pytest`-style, alongside the existing `backend/mcp_server/tests/` suite), against the fixed versioned dataset, not on live production traffic. `langfuse` dataset run, scoring every test case via the deterministic logic above, producing one aggregate score checked against the 0.9 threshold — fail the pipeline if it's not met. Live/online eval on real user questions is a possible future addition, not part of this pass.
6. **Regression detection** — re-run the same dataset after any change (new situations, prompt tweak, model swap) and diff scores against the previous run; a drop below 0.9, or any drop relative to the prior run even if still above 0.9, gets flagged — the same way the resume's "offline eval pipeline... detect LLM regressions before production deployment" describes. Track latency alongside correctness (Langfuse captures this automatically from the traces).
