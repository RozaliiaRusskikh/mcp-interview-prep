...
- [x] T038 [P] `backend/eval/dataset.json` — versioned eval dataset: router cases (question → expected tool name **and** arguments) and LLM-fallback cases (open-ended question → `expected_tool_calls` trajectory check + `expected_facts`/`must_not_contain` rubric), including adversarial/jailbreak prompts per `CLAUDE.md`'s "Prompt injection defense". Not yet synced to Langfuse (no Langfuse project/credentials exist yet — see T040)
- [x] T039 `backend/eval/run_eval.py` — deterministic scoring only, no LLM-as-judge, per the design above; also catches transient Gemini API errors (e.g. 503) per-case rather than crashing the whole suite
- [x] T040 `backend/eval/run_eval.py` pushes a per-case + aggregate score to Langfuse via `run_experiment` when `LANGFUSE_PUBLIC_KEY`/`LANGFUSE_SECRET_KEY` are set; degrades to local-only scoring (still fully functional, exit-code gate still works) when they aren't — true today, since no Langfuse project exists yet
- [x] T041 Verify: **done (2026-09-15)** — re-run twice against the current (now 28-case) dataset: 26/28 (92.9%) then 27/28 (96.4%), both clearing the 0.9 threshold. All 10 adversarial/jailbreak cases passed on both runs — the only misses each time were different (one transient Gemini 503, then two different LLM-fallback fact-phrasing near-misses that passed clean the next run), confirming the earlier 2026-08-17 non-determinism finding rather than a real regression. Router cases remain 100% deterministic across every run.
