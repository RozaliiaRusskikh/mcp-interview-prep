...
## Phase 5: Offline eval pipeline (Langfuse)
...
- [x] T036 [P] `backend/pyproject.toml` — add `langfuse` dependency
- [x] T037 Instrument `POST /chat` in `backend/main.py` with a Langfuse trace: question in → router decision → tool call + arguments → tool result → final answer out. Adversarial-style answers get a `declined_or_redirected` boolean score on the trace (renamed from the originally-proposed `prompt-injection-detected` — that name claimed the INPUT was classified as an attack, which nothing here does; this measures whether the ANSWER declined, a more honest label for what's actually checked)
- [x] T038 [P] `eval/dataset.json` — versioned eval dataset: router cases (question → expected tool name **and** arguments) and LLM-fallback cases (open-ended question → `expected_tool_calls` trajectory check + `expected_facts`/`must_not_contain` rubric), including adversarial/jailbreak prompts per `CLAUDE.md`'s "Prompt injection defense". Not yet synced to Langfuse (no Langfuse project/credentials exist yet — see T040)
- [x] T039 `eval/run_eval.py` — deterministic scoring only, no LLM-as-judge, per the design above; also catches transient Gemini API errors (e.g. 503) per-case rather than crashing the whole suite
- [x] T040 `eval/run_eval.py` pushes a per-case + aggregate score to Langfuse via `run_experiment` when `LANGFUSE_PUBLIC_KEY`/`LANGFUSE_SECRET_KEY` are set; degrades to local-only scoring (still fully functional, exit-code gate still works) when they aren't — true today, since no Langfuse project exists yet
- [ ] T041 Verify: **partially done, results below** — run repeatedly (2026-08-17) against the current backend. Router cases passed deterministically on every run. Along the way, fixed 2 real backend bugs (a `get_skill`-partial-match loophole that skipped a needed `get_experience` call; a rare degenerate-repetition failure — one question triggered 3,290 tool calls in a single turn — now capped) and 6 dataset over-strict checks (exact-phrase/date-format assertions that penalized correct, differently-phrased answers). Best clean run: 16/18 (88.9%), just under the 0.9 threshold. Score varied 66.7%–88.9% across 7 runs, partly from genuine Gemini API instability during testing (confirmed 503s on 2 separate runs) and partly from inherent LLM response non-determinism even at `temperature=0.1` — a single run's score is not a stable signal; re-run before trusting any one result. One finding needs a clean re-check once the API stabilizes: a Kubernetes fabrication case failed with the literal forbidden phrase present, a stronger signal than the false-positive pattern seen elsewhere in this run.
- [ ] T042 Gate deployment on T041 passing — mechanism is built (`run_eval.py` exits 1 below threshold, gateable by any CI/deploy step), but not yet exercised as an actual deploy gate since Phase 6 (deployment) hasn't started. Re-run after any change and diff against the previous Langfuse run before proceeding to Phase 6, once T041 is cleanly passing
